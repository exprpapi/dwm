...
static void moveresize(const Arg *arg) {
  // TODO: adjust window size to fit into screen area
  Client *c = selmon->sel;

  if (!(c && arg && arg->v && c->isfloating)) return;
  ...
  resize(c, x_new, y_new, w_new, h_new, True);
  
  for (XEvent ev; XCheckMaskEvent(dpy, EnterWindowMask, &ev);) {} // NOP
}
...
zoom(const Arg *arg)
{
  ...
  prevclient = nexttiled(selmon->clients);
	...
	if (c == nexttiled(selmon->clients))
		...
		if (!c || !(c = prevclient = nexttiled(c->next))) return;
	pop(c);
}
